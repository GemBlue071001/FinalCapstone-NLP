...
class Query2MainDB(DB_Handling_Base):
    ...
    def get_available_activity(self)->Union[bool, List[Dict[str,int]]]:
        ...
        activity_query = """SELECT "Id", "UserId", "JobPostId" FROM public."JobPostActivities" """
        try:
            ...
            print(f"Can't select JobPostActivities table in the database!", error)
            return False
    ...
    def update_score2table(self, update_data: List[Dict[str,Union[int,float]]]):
        ...
        update_query = """UPDATE public."JobPostActivities" SET "Score" = %s WHERE "Id" = %s"""
        try:
            ...
            print(f"Can't update JobPostActivities's score column in the database!", error)
            return False
...
class InsertDEMO2MainDB(DB_Handling_Base):
    ...
    def insertJobPostActivities(self, num_connections: int = 100):
        """
        ...
        conn, cur = self._get_cursor_and_connection()
        try:
            query = """INSERT INTO public."JobPostActivities" ("ApplicationDate","Status",
                                                                            "UserId", "JobPostId",
                                                                            "IsDeleted")
                                    VALUES (%s,%s,%s,%s,%s);"""
            ...
            conn.close()
            cur.close()
            print("Done insertJobPostActivities")
